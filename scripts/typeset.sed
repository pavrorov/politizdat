...
# Выносим пробелы и запятые из групп (курсив, жирный и пр.)
s/\([[:upper:][:lower:].,;]\)\(\\[[:upper:][:lower:]]\+\){[ \t]\+/\1 \2{/g;
s/\([.,;]*\)[ \t]\+}\([([:upper:][:lower:]]\|$\)/}\1 \2/g;
s/\([[:space:]]\+\)\(\\[[:upper:][:lower:]]\+{\)\(,[[:space:]]*\)/\3\1\2/g;
s/,}/},/g;
...
s/"\([^"]*\)"/\\qq{\1}/g;
s/\([«„]\|``\)/\\qq{/g; s/\([»”]\|''\)/}/g;
s/{\\textquotedbl}\([^{}]*\){\\textquotedbl}/\\qq{\1}/g;
s/[«„]/\\qq{/g; s/[»”“]/}/g;
...
# Наращение годов / веков
s/\([0-9IVXLCM]\+\)-\([[:lower:]]\{1,3\}}*\)[[:space:]]\+\(\(гг\|вв\)}*\.\)/\1-\2\\,\3/g;

# Короткий неразрывный пробел для годов и единиц измерения
s/\([0-9IVXLCM]\+}*\)[[:space:]~]*\(\(\\[[:upper:][:lower:]]\+{\)*\([БВГДЖЗКЛМНПРСТФХЦЧШЩбвгджзклмнпрстфхцчшщ]\+]\|гг\|вв\|тыс\|млн\|млрд\|трлн\)\.}*\)/\1~\2/g;
s/\([0-9IVXLCM]\+}*\)\(~\(\\[[:upper:][:lower:]]\+{\)*\([БВГДЖЗКЛМНПРСТФХЦЧШЩбвгджзклмнпрстфхцчшщ]\+]\|гг\|вв\|тыс\|млн\|млрд\|трлн\)\.}*\)*[[:space:]~]*\(\(\\[[:upper:][:lower:]]\+{\)*[БВГДЖЗКЛМНПРСТФХЦЧШЩбвгджзклмнпрстфхцчшщ]\+}*[\/.]\)/\1\2\\,\5/g;
s/\([0-9IVXLCM]\+}*\)\(~\(\\[[:upper:][:lower:]]\+{\)*\([БВГДЖЗКЛМНПРСТФХЦЧШЩбвгджзклмнпрстфхцчшщ]\+]\|гг\|вв\|тыс\|млн\|млрд\|трлн\)\.}*\)*[[:space:]~]*\(\(\\[[:upper:][:lower:]]\+{\)*[кМГмн]\?\([млгНВА]\|Вт\|Дж\|Па\)\(}*[\/.,;[:space:]]\|$\)\)/\1\2\\,\5/g;
s/\([0-9]\+\([\/,.][0-9]\)\?\)[[:space:]]*\\%/\1\\,\\%/g;

# Обычный неразрывный пробел для множителей
s/\([0-9IVXLCM]\+\)\\,\(\(тыс\|млн\|млрд\|трлн\)\.\)/\1~\2/g;

# Сокращения перед числами через неразрывный пробел
s/\(^\|[[:space:]]\+\|~\|[{([]\)\([Ии]зд\|[Сс]\|[Сс]тр\|[Тт]\|[Кк]н\|[Пп]п\?\|[Лл]\.[[:space:]]*д\|[[:upper:]]\?[[:lower:]]\{,2\}[бвгджзклмнпрстфхцчшщ]\)\.[[:space:]]\+\(\(\\[[:upper:][:lower:]]\+{\)*[0-9IVXLCM]\+\)/\1\2.~\3/g;
...
s/\([!?,.;]\)\+--/\1~--/g;
s/[[:space:]~]\+--/~--/g;
s/\([0-9IVXLCM]\+\(-[[:lower:]]\{1,3\}\)\?}*\)~---\([[:space:]]*\)\(\(\(начал[аео]\)[[:space:]]\+\)\?\(\\[[:lower:][:upper:]]\+{\)*[0-9IVXLCM]\+\(-[[:lower:]]\{1,3\}\)\?}*\)/\1~--\3\4/g;
s/^~---/---/;
s/---\([[:upper:][:lower:]]\)/--- \1/g;

# Не отрываем число от следующего за ним слова
s/\(^\|[[:space:](.,{~-]\|\\S\\,\|\\textnumero\\,\)\([0-9IVXLCM]\+\(-[[:lower:]]\{1,3\}\)\?}*\)[[:space:]]\+\([[:upper:][:lower:]-]\)/\1\2~\4/g;
# Сразу же восстанавливаем ошибочно связанные ссылки
s/\(\\[[:upper:][:lower:]]*cite\)\([[:space:]]*\[[^]]*\]\)\?\([[:space:]]\+\)\?{\([^}]*[0-9IVXLCM]\+\)}~\([^-]\)/\1\2\3{\4} \5/g;

# И наоборот, связываем число в скобках с предыдущим словом
s/\([0-9[:upper:][:lower:]]\)[[:space:]]\+\([0-9IVXLCM]\+\(-[[:lower:]]\{1,3\}\)\?}*\))/\1~\2)/g;

# Связываем имена в скобках с цитатой
s/[[:space:]]\+\(([[:space:]]*[[:upper:]]\)/~\1/g;
s/[[:space:]]\+\(([[:space:]]*\\\(emph\|acc\|textit\|textbf\){[[:upper:]]\)/~\1/g;

# Восстанавливаем тире после ссылок
s/\(\\[[:lower:]]*cite\(\[[^]]\+\]\)\?{[^}]\+[0-9IVXLCM]\+}\)[[:space:]]*--\([[:space:]]\+\|$\)/\1~---\3/g;

# Не отрываем нумерацию
s/\(^\|[[:space:]{]\|\\ldots{}\|…\|\.\.\.\)\(\([0-9IVXLCM]\{1,2\}\(-[[:lower:]]\+\)\?[).]\|[A-Za-zА-Яа-я])\)}\?\)[[:space:]]\+/\1\2~/g;
#s/\(^\|[[:space:]]\|\\ldots{}\|…\|\.\.\.\)\(\\[[:upper:][:lower:]]\+\)\?{\([0-9IVXLCM]\{1,2\}\(-[[:lower:]]\+\)\?[).]\)}[[:space:]]\+/\1\2{\3}~/g;
s/\([и]\)~\([0-9IVXLCMA-Za-zА-Яа-я])\)/\1 \2/g;
...
s/\(^\|[[:space:](]\+\|{\)\(\([Сс]м\|[Пп]л\|[Уу]л\|Тов\|[Тт]т\|[Тт]\.т\|Гос\|[Гг]г\|[Гг]\.г\|[Мм]м\|[Мм]\.м\|[Оо]бв\|[Сс]р\|[Пп]роф\|[Аа]кад\|[Ии]нж\|[Рр]ед\|им\)\.}\?\)[[:space:]]\+/\1\2~/g;
